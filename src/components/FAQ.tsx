  ...
  return (
    <section id="faq" className="py-24 md:py-32 bg-white">
      <div className="container mx-auto px-4 md:px-8">
        ...
        {/* --- FAQ ACCORDION (COMPACT MODE) --- */}
        <div className="max-w-2xl mx-auto">
          {/* space-y-2: Jarak antar item dirapatkan */}
          <Accordion type="single" collapsible className="space-y-2">
            {faqs.map((faq, idx) => (
              <AccordionItem
                key={idx}
                value={`item-${idx}`}
                // Compact Style: rounded-2xl, padding lebih kecil
                className="bg-[#F7F0E6]/30 border border-primary/10 rounded-2xl px-5 md:px-6 overflow-hidden transition-all duration-300 hover:border-primary/30 animate-fade-up data-[state=open]:bg-[#F7F0E6]/60"
                style={{ animationDelay: `${idx * 0.05}s` }} // Animasi lebih cepat dikit
              >
                {/* Trigger: py-3 (Ramping), text-sm (Compact Font) */}
                <AccordionTrigger className="text-left font-sans font-medium text-primary text-sm md:text-base hover:no-underline py-3 md:py-4 data-[state=open]:text-emerald-800">
                  {faq.question}
                </AccordionTrigger>
                
                {/* Content: pb-3, text-xs (Compact Font) */}
                <AccordionContent className="text-primary/70 font-sans font-light text-xs md:text-sm pb-4 leading-relaxed whitespace-pre-line">
                  {faq.answer}
                </AccordionContent>
              </AccordionItem>
            ))}
